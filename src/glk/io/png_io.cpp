...
bool load_png(FILE* fp, int& width, int& height, std::vector<unsigned char>& bytes) {
  ...
  // PNG_TRANSFORM_EXPAND converts palette->RGB and expands tRNS chunks to full alpha channel
  png_read_png(png, info, PNG_TRANSFORM_PACKING | PNG_TRANSFORM_STRIP_16 | PNG_TRANSFORM_EXPAND, nullptr);
