...
std::shared_ptr<BasicProjectionControl> LightViewerContext::use_perspective_projection_control(float fovy_deg, float near_, float far_) {
  auto proj = std::make_shared<guik::BasicProjectionControl>(canvas->size);
  proj->set_projection_mode(0);
  proj->set_fovy(fovy_deg);
  proj->set_depth_range(Eigen::Vector2f(near_, far_));
  canvas->projection_control = proj;
  return proj;
}

std::shared_ptr<BasicProjectionControl> LightViewerContext::use_orthographic_projection_control(float width, float near_, float far_) {
  auto proj = std::make_shared<guik::BasicProjectionControl>(canvas->size);
